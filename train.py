...
from utils.logger import count_parameters
...
def setup_deterministic(seed=42):
    print(f"[Init] Setting deterministic mode with seed = {seed}")
    random.seed(seed)
    np.random.seed(seed)
...
def save_run_summary(cfg, args, output_dir):
    log_path = os.path.join(output_dir, "run_summary.txt")
    with open(log_path, "w") as f:
        f.write(f"=== Run Summary ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}) ===\n")
        f.write(f"Seed: 54\nPrecision: {cfg.MODEL.PREC}\n")
        f.write(f"Backbone: {cfg.MODEL.BACKBONE.NAME}\n")
        f.write(f"Optimizer: {cfg.OPTIM.NAME} (LR={cfg.OPTIM.LR}, Scheduler={cfg.OPTIM.LR_SCHEDULER})\n")
        f.write(f"Input Size: {cfg.INPUT.SIZE}\n")
        f.write(f"Dataset: {cfg.DATASET.NAME} ({cfg.DATASET.ROOT})\n")
        f.write(f"Batch Size: {cfg.DATALOADER.TRAIN_X.BATCH_SIZE}\n")
        f.write(f"Warmup Epochs: {cfg.OPTIM.WARMUP_EPOCH}\n")
        f.write(f"Output Dir: {output_dir}\n")
        f.write("=" * 50 + "\n")
    print(f"[Log] Run summary saved at: {log_path}")
...
def main():
    ...
    for name, param in model.named_parameters():
        if any(k in name for k in ["image_encoder", "visual", "text_encoder", "token_embedding"]):
    ...
    ########## for non-baseline  ##########
        ...
        try:
            ...
            prompt_params = model.module.prompt_params()
        prompt_group = {'params': prompt_params, 'lr': cfg.OPTIM.LR * 0.1}
        # prompt_group = {'params': prompt_params, 'lr': 0.005}
        ...
        # fusion_weitghts
        try:
            fusion_weights_params = model.fusion_weights_params()
        except:
            fusion_weights_params = model.module.fusion_weights_params()

        fusion_weights_group = {'params': fusion_weights_params, 'lr': cfg.OPTIM.LR * 0.1}

        optim_polices = [prompt_group, text_group, fusion_weights_group]
    ...
    if cfg.TRAINER.FINETUNE_BACKBONE:
        ...
        optim_polices.append(base_group)
    ...
    if cfg.TRAINER.FINETUNE_ATTN:
        ...
        optim_polices.append(attn_group)

    # optim = torch.optim.SGD(optim_polices, lr=cfg.OPTIM.LR,
    ...
    # -> AdamW, RAdam
    optim = torch.optim.AdamW(optim_polices, lr=cfg.OPTIM.LR, weight_decay=cfg.OPTIM.WEIGHT_DECAY)
    ...
    count_parameters(model, optim)

    save_run_summary(cfg, args, log_folder)
    ...
    print("================================")
    print(f"lr: {cfg.OPTIM.LR} prompt_lr: {cfg.OPTIM.LR * 0.1} fusion_weights_lr:{cfg.OPTIM.LR * 0.1}")
    # print(f"lr: {cfg.OPTIM.LR} prompt_lr: 0.005")
    print("================================")
    print("===============================", file=logfile, flush=True)
    print(f"lr: {cfg.OPTIM.LR} prompt_lr: {cfg.OPTIM.LR * 0.1} fusion_weights_lr:{cfg.OPTIM.LR * 0.1}", file=logfile, flush=True)
    # print(f"lr: {cfg.OPTIM.LR} prompt_lr: 0.005", file=logfile, flush=True)
    print("================================", file=logfile, flush=True)
    ...
    for epoch in range(args.start_epoch, cfg.OPTIM.MAX_EPOCH):
        batch_time, losses, mAP_batches, grad_mean, grad_max = train_coop(train_loader, [val_loader], model, optim,
                                                                          sched, args, cfg, epoch)
        print('Train: [{0}/{1}]\t'
              'Time {batch_time.avg:.3f}\t'
              'Loss {losses.avg:.2f} \t'
              'mAP {mAP_batches.avg:.2f}\t'
              'GradMean {grad_mean:.4f}\tGradMax {grad_max:.4f}'.format(
            epoch + 1, cfg.OPTIM.MAX_EPOCH, batch_time=batch_time,
            losses=losses, mAP_batches=mAP_batches,
            grad_mean=grad_mean, grad_max=grad_max), flush=True)
