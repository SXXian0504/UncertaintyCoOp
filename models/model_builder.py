...
def build_model(cfg, args, classnames):
    ...
    functions = {
        ...
        "baseline": baseline,
        "uncertaintycoop": uncertaintycoop
    }
    ...
    if not args.evaluate:
        arch_name += "-{}-{}-bs{}-e{}-p{}-lr{}".format(args.method_name, cfg.OPTIM.LR_SCHEDULER,
                                                       cfg.DATALOADER.TRAIN_X.BATCH_SIZE, cfg.OPTIM.MAX_EPOCH,
                                                       args.partial_portion, args.lr)
        arch_name += f"_{datetime.now().strftime('%Y%m%d-%H%M%S')}"
